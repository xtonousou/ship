...
function check_cidr() {
  ...
  case "${2}" in
    "ipv4")
      ...
      if [[ "${1}" -lt 1 ]] || [[ "${1}" -gt 32 ]]; then error_exit "${DIALOG_NO_VALID_CIDR}"; fi
    ;;
  esac
}
...
function check_ip_address() {
  ...
  elif [[ "${PART_A}" -lt 0 ]] || [[ "${PART_A}" -gt 255 ]] || [[ "${PART_B}" -lt 0 ]] || [[ "${PART_B}" -gt 255 ]] || [[ "${PART_C}" -lt 0 ]] || [[ "${PART_C}" -gt 255 ]] || [[ "${PART_D}" -lt 0 ]] || [[ "${PART_D}" -gt 255 ]]; then
    show_usage_subnet
    error_exit
  fi
